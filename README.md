...
Structured logs are stored per day and model. They include timestamps, public alias, origin/provider IP, status, TTFT, total duration, token usage, guardrail result, and sanitized effective parameters after precedence rules. API keys, authorization headers, and routing secrets are redacted. Detail rows and complete-day KPIs use independent requests: the 250 newest rows can render immediately while SQLite calculates the exact hourly histogram and totals. KPI results are cached for 30 seconds during automatic refreshes; a manual refresh bypasses the cache. Existing rows stay visible while either request is in flight, avoiding table flicker under heavy load. Large embedding vectors are compacted before storage because their individual coordinates provide no operational value. Excel export remains available on demand. A separate LiteLLM tab exposes stdout/stderr for startup and provider diagnostics.
...
Each model has structured daily logs containing request/response summaries, the effective sanitized provider parameters, status, origin IP, provider IP, TTFT, total duration, token counts, and guardrail outcome. This records the final OpenAI/vLLM/NIM/Workbench/Triton-compatible options after precedence rules have run, without logging API keys, authorization headers, messages as parameters, or routing secrets. Token counters are stored in scalar columns for efficient aggregation, and oversized numeric arrays such as embedding vectors are reduced to a short preview. The dashboard loads at most 250 recent detail rows independently from exact full-day SQLite KPIs and the hourly histogram, caches the expensive aggregation for 30 seconds during automatic refresh, keeps rendered rows visible while refreshing, rejects stale responses, and can export the selected day to Excel.
