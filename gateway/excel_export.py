...
def build_logs_xlsx(model: str, day: str, rows: list[dict[str, Any]], kpis: dict[str, Any]) -> bytes:
    ...
    headers = ["Fecha Madrid", "Estado", "Origen", "Proveedor", "TTFT ms", "Duración ms", "Guardrail", "Motivo guardrail", "Parámetros efectivos", "Pregunta / entrada", "Respuesta / salida", "Error"]
    header_row = 15
    ...
    for row_number, item in enumerate(rows, start=header_row + 1):
        ...
        parameters = json.dumps(item.get("parameters"), ensure_ascii=False, default=str)
        values = [item.get("started_at") or item.get("created_at"), item.get("status"), item.get("origin_ip"),
                  ...
                  item.get("guardrail_reason"), parameters, request, response, item.get("error")]
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"><pane ySplit="15" topLeftCell="A16" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols><col min="1" max="1" width="24" customWidth="1"/><col min="2" max="8" width="18" customWidth="1"/><col min="9" max="12" width="48" customWidth="1"/></cols><sheetData>{''.join(xml_rows)}</sheetData><autoFilter ref="A15:L{last_row}"/></worksheet>'''
