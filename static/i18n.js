...
(() => {
  ...
  const rows = [
    ...
    ["On-premise 7.3.2+ · credencial Knox", "On-premises 7.3.2+ · Knox credential", "On-premise 7.3.2+ · credenziale Knox"],
    ["Usa una credencial emitida para el servicio seleccionado. Los JWT caducan; las API keys Knox son largas y revocables, pero AI Inference sólo las acepta si un administrador ha habilitado la preautenticación por API key en el Knox del Data Lake.", "Use a credential issued for the selected service. JWTs expire; Knox API keys are long-lived and revocable, but AI Inference only accepts them when an administrator has enabled API-key pre-authentication in the Data Lake Knox service.", "Usa una credenziale emessa per il servizio selezionato. I JWT scadono; le API key Knox sono di lunga durata e revocabili, ma AI Inference le accetta solo se un amministratore ha abilitato la preautenticazione tramite API key nel Knox del Data Lake."],
    ...
    ["IA Gateway detecta la caducidad de los JWT. Sólo muestra renovación automática cuando dispone de un mecanismo no interactivo compatible; para claves opacas muestra “caducidad no declarada” y recomienda verificar su política en Cloudera.", "IA Gateway detects JWT expiry. It only shows automatic renewal when a compatible non-interactive mechanism is available; opaque keys show “expiry not declared” and should be checked in Cloudera.", "IA Gateway rileva la scadenza dei JWT. Mostra il rinnovo automatico solo quando è disponibile un meccanismo non interattivo compatibile; per le chiavi opache mostra “scadenza non dichiarata” e consiglia di verificarne la policy in Cloudera."],
