...
(() => {
  ...
  const rows = [
    ...
    ["Ollama: -1 mantiene el modelo cargado, 0 lo descarga o usa una duración como 5m.", "Ollama: -1 keeps the model loaded, 0 unloads it, or use a duration such as 5m.", "Ollama: -1 mantiene il modello caricato, 0 lo scarica oppure usa una durata come 5m.", "Ollama : -1 maintient le modèle chargé, 0 le décharge, ou utilisez une durée telle que 5m."],
    ...
    ["Los modelos de chat se validan contra el texto final esperado; los embeddings, comprobando que devuelven un vector numérico válido.", "Chat models are checked against the expected final text; embeddings are checked for a valid numeric vector.", "I modelli chat vengono verificati rispetto al testo finale previsto; gli embedding controllando che restituiscano un vettore numerico valido.", "Les modèles de chat sont vérifiés par rapport au texte final attendu ; les embeddings en contrôlant qu’ils renvoient un vecteur numérique valide."],
    ["Tokens/s como KPI principal", "Tokens/s as the primary KPI", "Token/s come KPI principale", "Tokens/s comme KPI principal"],
    ["Errores, corrección y concurrencia sostenible", "Errors, correctness, and sustainable concurrency", "Errori, correttezza e concorrenza sostenibile", "Erreurs, exactitude et concurrence soutenable"],
    ...
    ["TOKENS / SEGUNDO", "TOKENS / SECOND", "TOKEN / SECONDO", "TOKENS / SECONDE"], ["Rendimiento agregado procesado", "Aggregate processed throughput", "Rendimento aggregato elaborato", "Débit agrégé traité"],
    ["Tokens/s", "Tokens/s", "Token/s", "Tokens/s"], ["TOKENS/S EN VIVO", "LIVE TOKENS/S", "TOKEN/S IN TEMPO REALE", "TOKENS/S EN DIRECT"],
    ["Entrada procesada", "Processed input", "Input elaborato", "Entrée traitée"], ["Salida generada", "Generated output", "Output generato", "Sortie générée"], ["estimado", "estimated", "stimato", "estimé"],
    ["ÚLTIMO", "LATEST", "ULTIMO", "DERNIER"], ["Sin medición anterior", "No previous measurement", "Nessuna misurazione precedente", "Aucune mesure précédente"],
    ["tokens de entrada", "input tokens", "token di input", "tokens d’entrée"], ["tokens de salida", "output tokens", "token di output", "tokens de sortie"],
    ["Chat · texto final esperado", "Chat · expected final text", "Chat · testo finale previsto", "Chat · texte final attendu"],
    ["Vector numérico finito · tokens de entrada", "Finite numeric vector · input tokens", "Vettore numerico finito · token di input", "Vecteur numérique fini · tokens d’entrée"],
    ...
    ["Cloudera AI Workbench App", "Cloudera AI Workbench App", "Cloudera AI Workbench App", "Application Cloudera AI Workbench"],
    ["Conecta AI Inference, Model Service de Workbench y Workbench Apps con API OpenAI compatible.", "Connect AI Inference, Workbench Model Service, and Workbench Apps with an OpenAI-compatible API.", "Collega AI Inference, Model Service di Workbench e Workbench App con API compatibile OpenAI.", "Connectez AI Inference, Model Service de Workbench et les applications Workbench avec une API compatible OpenAI."],
    ["Workbench App · API OpenAI compatible", "Workbench App · OpenAI-compatible API", "Workbench App · API compatibile OpenAI", "Application Workbench · API compatible OpenAI"],
    ["Conecta una aplicación que publique directamente", "Connect an application that directly exposes", "Collega un'applicazione che espone direttamente", "Connectez une application qui expose directement"],
    ["Se conserva el streaming real; la API key Bearer es opcional.", "Real streaming is preserved; the Bearer API key is optional.", "Lo streaming reale viene mantenuto; l'API key Bearer è opzionale.", "Le streaming réel est conservé ; la clé API Bearer est facultative."],
    ["Modelo remoto", "Remote model", "Modello remoto", "Modèle distant"],
    ["Valor exacto que la aplicación espera en el campo", "Exact value expected by the application in the", "Valore esatto atteso dall'applicazione nel campo", "Valeur exacte attendue par l'application dans le champ"],
    ["API key Bearer", "Bearer API key", "API key Bearer", "Clé API Bearer"],
    ["Autenticación", "Authentication", "Autenticazione", "Authentification"],
    ["Pública · sin credencial", "Public · no credential", "Pubblica · nessuna credenziale", "Publique · sans identifiant"],
    ["Bearer API key", "Bearer API key", "API key Bearer", "Clé API Bearer"],
    ["Vacío al editar conserva la actual", "Leave blank while editing to keep the current key", "Lascia vuoto durante la modifica per mantenere la chiave attuale", "Laissez vide lors de la modification pour conserver la clé actuelle"],
    ["Se enviará como", "It will be sent as", "Verrà inviata come", "Elle sera envoyée comme"],
    ["Seleccionar «Pública» elimina cualquier clave anterior.", "Selecting “Public” removes any previous key.", "Selezionando “Pubblica” viene eliminata qualsiasi chiave precedente.", "Sélectionner « Publique » supprime toute clé précédente."],
    ["Permite avisar antes de rotar una API key opaca.", "Enables a warning before rotating an opaque API key.", "Consente di avvisare prima di ruotare un'API key opaca.", "Permet d'avertir avant la rotation d'une clé API opaque."],
    ["Validación TLS", "TLS verification", "Verifica TLS", "Vérification TLS"],
    ["Endpoint OpenAI completo de la app; también se acepta la URL base terminada en /v1.", "Full OpenAI endpoint for the app; a base URL ending in /v1 is also accepted.", "Endpoint OpenAI completo dell'app; è accettato anche un URL base che termina in /v1.", "Endpoint OpenAI complet de l'application ; une URL de base se terminant par /v1 est également acceptée."],
    ["App pública · autenticación no requerida", "Public app · authentication not required", "App pubblica · autenticazione non richiesta", "Application publique · authentification non requise"],
    ["API key Bearer · rotación manual", "Bearer API key · manual rotation", "API key Bearer · rotazione manuale", "Clé API Bearer · rotation manuelle"],
    ["Sin autenticación", "No authentication", "Nessuna autenticazione", "Sans authentification"],
    ["App pública", "Public app", "App pubblica", "Application publique"],
    ["API key de la app disponible", "App API key available", "API key dell'app disponibile", "Clé API de l'application disponible"],
    ["Workbench App · OpenAI", "Workbench App · OpenAI", "Workbench App · OpenAI", "Application Workbench · OpenAI"],
    ["API key de Workbench App", "Workbench App API key", "API key della Workbench App", "Clé API de l'application Workbench"],
    ...
    ["Comprobación periódica", "Periodic check", "Controllo periodico", "Vérification périodique"],
    ["Pendiente · cada 5 min", "Pending · every 5 min", "In attesa · ogni 5 min", "En attente · toutes les 5 min"],
    ["Sin detalle", "No details", "Nessun dettaglio", "Aucun détail"],
  ...
  const patterns = {
    en: [
      ...
      [/^Operativo · (\d+) ms$/, "Operational · $1 ms"], [/^Enviando… (\d+) s · esperando primera respuesta$/, "Sending… $1 s · waiting for first response"],
    ...
    fr: [
      ...
      [/^Operativo · (\d+) ms$/, "Opérationnel · $1 ms"], [/^Enviando… (\d+) s · esperando primera respuesta$/, "Envoi… $1 s · attente de la première réponse"],
    ...
    it: [
      ...
      [/^Operativo · (\d+) ms$/, "Operativo · $1 ms"], [/^Enviando… (\d+) s · esperando primera respuesta$/, "Invio… $1 s · attesa della prima risposta"],
